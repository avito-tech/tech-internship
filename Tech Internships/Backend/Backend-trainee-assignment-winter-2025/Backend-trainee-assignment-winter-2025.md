...
* Для авторизации доступов должен использоваться JWT. После регистрации пользователя ему выдется токен.  Все остальные действия подтверждаются этим токеном. При первой авторизации пользователь должен создаваться автоматически.
